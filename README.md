...
* Added a sorting function to `service/tags_manager.php`. When called, it will sort a tag list alphabetically (per localization rules), and then secondarily apply human-friendly "natural" sorting of numeric tags. This is presently tied to global localization settings; I'm unaware of a way to do this using user-specific language preferences of different board users, because the variable holding that information is just simple language codes like `en` or `ru` that do not correspond to localization names used by PHP (e.g. `en_US.UTF-8`). Anyway, this sorting has presently been applied to the all-tags list provided by ACP &gt; Extensions &gt; RH Topic Tags &gt; Manage Tags. Not yet applied to tags shown on topics, on posts, or on tag cloud.
...
* Added `migrations/release_1_0_3.php`, and `migrations/release_3_0_0.php` through `migrations/release_3_0_2.php` files that should account for any other variants people have installed. Not sure what purpose these things serve, but them being missing would presumably be problematic somehow.
...
* There's still a less serious privacy/security problem with the tag cloud: Tags that are used (presently) only in a forum to which the logged-in user does not have access will still be shown in the tag cloud. The usage count for a tag in the cloud (if the count feature is on) will include instances in forums to which that user does not have access. The first of exposes internal-only tags to those who should not know them, and the second reveals to the user that there is hidden content they don't have access to. In both cases, clicking on such a tag will *not* show posts, titles of posts, or even forum names that the user can't access, so it's not as bad as it could be. See `adm/style/manage_tags.js`, which has various checks in it, for possibly how to approach this (though I do not know why this core functionality file is in a `style/` directory, since it appears to have nothing to do with themes and other presentation).
* Going to `https://yoursite.com/tags/` does not work, only `https://yoursite.com/tags` with no trailing slash, but these URLs canonically should be treated as equivalent. There is a `https://yoursite.com/tags/suggest` page (loading a script that takes data from another, and is used by various functions), so `https://yoursite.com/tags/` does actually exist.
* The tags shown on a post are alphabetized in "computerese" style, with "A", "B", "C", etc. grouped before "a", "b", "c", etc. This is undesirable as it is confusing for human users. I may be looking for `loadTags` function.
...
* There appears to be a significant amount of code in here (e.g. in `service/tags_manager.php`) to support an option for case-sensitive tagging (or at least case-sensitive sorting and other display considerations regarding tagging; the full intent isn't clear yet). But there is no ACP option to enable such a thing, it's always defaulting to "false" or even hard-coded that way in a few places (i.e. to be case-insensitive).
 * Case-sensitivity of actual tags would probably be undesirable for most use-cases (e.g. if someone creates a "YouTube" tag and someone else writes it as "Youtube" or "youtube" or "YOUTUBE" you end up with redundant tags, and this is especially likely on a busy board with many topics and users). But a niche recipe site with tagging controlled by only a few editors, for example, might want to distinguish between Turkey the country and turkey the bird or between Chile the country and chile the pepper.
 * Case-sensitivity of display of tags in a list, however, is good. Something best done (to the extent feasible) in compliance with localization (different languages alphabetize differently, especially with regard to characters with diacritics). The original behavior seems to have been to do no sorting in some places (accept database-entered tag order), and to sort in "computer-friendly" manner in others (A, B, C, etc. grouped before a, b, c, etc., and human-unfriendly number handling that treats 02 as less than 1. This "user-hateful" style has been fixed at ACP &gt; Extensions &gt; RH Topic Tags &gt; Manage Tags (list of all tags), but not yet elsewhere, such as in the tags listed at a topic, or in tag cloud).
 * Tag cloud in particular is sorting things in order of most to least frequently used, which makes the sizing stuff it is doing redundant. Tag clouds are typically alpha order with the size indicating frequency.
 * Making any/all of this work might be very easy or might be a massive endeavor, depending on the development and testing state of the code relating to this, which is still under some investigation. Part of it may involve changing `@casesensitive` into a class property addressed as `@this->casesensitive`.
* In `service/tags_manager.php` there is a "Note: that $query needs to be at least 3 characters long", and code like `if (utf8_strlen($query) < 3)`; this may have to be changed to support 2- and 1-character tags. Depends on exactly what circumstances this 3-char. constraint is being applied to. So far, it looks like this only affects `get_tag_suggestions()`, but it's not clear that this function actually aligns well with its name (i.e. it may not be limited to the tag-suggestion feature of trying to match existing tags when you enter a few characters).
