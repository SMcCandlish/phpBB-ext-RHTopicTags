...
$lang = array_merge($lang, array(
	...
	'TOPICTAGS_PRUNE_INVALID_TAGS_CONFIRM'		=> 'Это действие <strong>удалит</strong> все теги, которые не соответвуют установленным регулярным выражениям и может удалить полезные теги, если вы не будете предельно осторожны!',
	...
	'TOPICTAGS_ALLOWED_TAGS_REGEX_EXP'			=> '<strong>Внимание</strong>: Не изменяйте значение, если вы не уверены в своих действиях. <em>Теги могут содержать максимум 30 символов, а разделителем между ними в базе данных является «/», поэтому этот символ не должен добавляться в регулярное выражение (отдельно или как часть класса), чтобы его можно было использовать в тегах.</em><br />Обратите внимание, что после изменения регулярных выражений недействительные теги по-прежнему будут отображаться в темах, но исключаются из поиска по тегам.<br />Рассмотрим удаление неправильных тегов (см. секцию Ехобслуживания).<br />По-умолчанию: ' . $lang['ACP_RH_TOPICTAGS_REGEX_DEFAULT'],

	'TOPICTAGS_CONVERT_SPACE_TO_MINUS'			=> 'Преобразование « » в «-»',
	'TOPICTAGS_CONVERT_SPACE_TO_MINUS_EXP'		=> 'Если установлено «Да», все пробелы (« ») автоматически заменяются дефисами («-»).<br /><strong>Примечание 1</strong>: В регулярном выражении необходимо разрешить «-»; в противном случае теги с пробелами будут отклонены.<br /><strong>Примечание 2</strong>: Существующие теги с пробелами не будут преобразованы автоматически.',
